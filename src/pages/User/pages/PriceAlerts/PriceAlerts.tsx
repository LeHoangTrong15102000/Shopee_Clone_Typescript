  ...
  return (
    <div className='rounded-sm bg-white px-4 pb-10 shadow md:px-7 md:pb-20 dark:bg-slate-800'>
      ...
      <div className='border-b border-b-gray-200 py-6 dark:border-b-slate-700'>
        <h1 className='text-lg font-medium capitalize text-gray-900 dark:text-gray-100'>Thông báo giá</h1>
              ...
              >
                <div className='min-w-0 flex-1'>
                  <div className='flex items-center gap-2'>
                    <span className='text-sm font-medium text-gray-900 dark:text-gray-100'>Sản phẩm: {alert.product_id}</span>
                    <span className={`rounded-full px-2 py-0.5 text-xs ${alert.is_active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400'}`}>
                      {alert.is_active ? 'Đang theo dõi' : 'Đã kích hoạt'}
                    </span>
                  </div>
                  <div className='mt-1 flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400'>
                    <span>Giá mục tiêu: <span className='font-medium text-[#ee4d2d]'>₫{formatCurrency(alert.target_price)}</span></span>
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50' role='dialog' aria-modal='true'>
          <div className='mx-4 w-full max-w-sm rounded-lg bg-white p-6 shadow-xl dark:bg-slate-800'>
            ...
            <div className='mt-4 flex justify-end gap-3'>
              ...
              <Button onClick={() => deletingId && deleteMutation.mutate(deletingId)} disabled={deleteMutation.isPending} className='rounded bg-red-500 px-4 py-2 text-sm text-white hover:bg-red-600 disabled:opacity-50'>
