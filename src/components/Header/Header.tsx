...
const Header = () => {
  ...
  // Xử lý click outside để đóng search suggestions
  useEffect(() => {
  ...
  const handleBlurInput = () => {
    // Thêm delay để cho phép click vào suggestions trước khi đóng
    setTimeout(() => {
  ...
  const handleSearchSubmit = (searchTerm?: string) => {
    ...
    const config = queryConfig.order
      ? omit({ ...queryConfig, name: finalSearchValue }, ['order', 'sort_by'])
      : { ...queryConfig, name: finalSearchValue }

    navigate({
      pathname: path.products,
      search: createSearchParams(config as FormData).toString()
    })
  ...
  return (
    <div className='bg-[linear-gradient(-180deg,#f53d2d,#f63)] pb-[25px] pt-[4px] text-white'>
      <div className='container'>
        ...
        {/* Grid chia 4 phần: 1 logo shopee, 1 thanh search, 1 cart, 1 notifications + user */}
        <div className='mt-2 md:mt-4 grid grid-cols-12 items-end gap-2 md:gap-4'>
          {/* Icon shopee - Responsive width */}
          <Link to={path.home} className='col-span-2 md:col-span-2' onClick={handleRefresh}>
            <svg viewBox='0 0 192 65' className='h-8 w-full fill-white md:h-10 lg:h-12'>
              <g fillRule='evenodd'>
          ...
          {/* Search với SearchSuggestions - Responsive width */}
          <div className='col-span-8 md:col-span-9 relative' ref={searchContainerRef}>
            <form onSubmit={handleFormSubmit}>
              <div className='flex rounded-sm bg-white p-1'>
                <input
                  ...
                  value={searchValue}
                  type='text'
                  className='flex-grow border-none bg-transparent px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm text-[rgba(0,0,0,.95)] outline-none'
          ...
          {/* Cart - Ngay bên cạnh thanh search */}
          <div className='col-span-1 flex items-center justify-center'>
            <div className='relative'>
              <Popover
                className=''
                renderPopover={
                  <div className='relative max-w-[280px] md:max-w-[400px] rounded-sm border border-gray-200 bg-white text-sm shadow-md'>
                    {purchasesInCart && purchasesInCart.length > 0 ? (
                      <div className='py-[10px] pl-[10px]'>
                        <div className='capitalize text-[rgba(0,0,0,.26)]'>sản phẩm mới thêm</div>
                        <div className='mt-5'>
                          {purchasesInCart.slice(0, MAX_PURCHASES).map((purchase) => (
                            <div className='mt-2 flex py-2 pr-2 hover:bg-gray-100' key={purchase._id}>
                              ...
                              <div className='ml-2 flex-grow overflow-hidden'>
                                <div className='truncate text-xs md:text-sm'>{purchase.product.name}</div>
                              </div>
                        ...
                        <div className='mt-6 flex items-center justify-between text-gray-500'>
                          ...
                          <Link
                            to={path.cart}
                            className='rounded-sm bg-[#ee4d2d] px-3 md:px-4 py-1 md:py-2 capitalize text-white text-xs md:text-sm hover:bg-opacity-90'
                          >
                            xem giỏ hàng
                          </Link>
                        </div>
                      ...
                      <div className='flex h-[200px] md:h-[250px] w-[280px] md:w-[400px] flex-grow flex-col items-center justify-center p-2'>
                        ...
                        <span className='mt-5 capitalize text-black/80 text-xs md:text-sm'>Chưa có sản phẩm</span>
                      </div>
                    )}
                  </div>
                }
          ...
          {/* Notifications + User Menu - Responsive width */}
          <div className='col-span-2 md:col-span-1 flex items-center justify-end space-x-2 md:space-x-3'>
                ...
                renderPopover={
                  <div className='relative rounded-sm border border-gray-200 shadow-md transition-all'>
                    ...
                    >
                      <Link
                        to={path.profile}
                        className='block w-full bg-white py-3 px-4 text-left hover:bg-slate-100 hover:text-cyan-500 text-sm text-gray-800'
                      >
                        Tài Khoản Của Tôi
                      </Link>
                      <Link
