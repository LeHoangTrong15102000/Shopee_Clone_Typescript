...
const BankTransferPayment = memo(function BankTransferPayment({
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className='space-y-6'>
        ...
        <div className='space-y-4'>
          ...
          <BankDropdown selectedBank={selectedBank} onSelectBank={handleSelectBank} isOpen={isDropdownOpen} onToggle={handleToggleDropdown} />
        </div>
        {selectedBank && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className='pt-2'>
            <Button type='button' onClick={handleProceedToPayment} className='w-full rounded-xl bg-orange px-6 py-3 font-medium text-white hover:bg-orange/90'>
              Tiếp tục
            </Button>
          </motion.div>
        )}
  ...
  if (paymentState === 'expired') {
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className='flex flex-col items-center space-y-4 py-8'>
        ...
        <div className='text-center'>
          ...
          <p className='mt-1 text-sm text-gray-500 dark:text-gray-400'>Thời hạn thanh toán đã hết. Vui lòng đặt hàng lại.</p>
        </div>
      </motion.div>
    )
  }
