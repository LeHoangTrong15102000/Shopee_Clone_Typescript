  ...
  return (
    <div className='space-y-4 md:space-y-6'>
      <div className='flex justify-center'>
        <VisualCardPreview cardNumber={formattedCardNumber} cardHolder={cardHolder} expiryDate={formattedExpiry} cardType={cardType} cvv={cvv} isFlipped={isCardFlipped} />
      </div>
      ...
      <div className='space-y-3 md:space-y-4'>
        <div>
          ...
          <motion.div className='relative' animate={shakeFields.cardNumber ? 'shake' : ''} variants={shakeAnimation}>
            <Input
              type='text' placeholder='1234 5678 9012 3456' register={register} name='cardNumber'
              ...
              classNameInput={`w-full rounded-lg border px-2 py-2 md:px-3 md:py-3 pr-16 md:pr-20 focus:outline-hidden ${
                validationState.cardNumber.touched
                  ? validationState.cardNumber.isValid ? 'border-green-500 focus:border-green-500' : 'border-red-500 focus:border-red-500'
                  : 'border-gray-300 focus:border-orange'
              }`}
              maxLength={cardType === 'amex' ? 17 : 19} autoComplete='cc-number' onBlur={handleCardNumberBlur}
            />
        ...
        <div>
          ...
          <Input
            type='text' placeholder='NGUYEN VAN A' register={register} name='cardHolder'
        ...
        <div className='grid grid-cols-1 gap-3 sm:grid-cols-2 md:gap-4'>
          <div>
            ...
            <motion.div className='relative' animate={shakeFields.expiryDate ? 'shake' : ''} variants={shakeAnimation}>
              <Input
                type='text' placeholder='MM/YY' register={register} name='expiryDate'
                ...
                classNameInput={`w-full rounded-lg border px-2 py-2 md:px-3 md:py-3 pr-8 md:pr-10 focus:outline-hidden ${
                  validationState.expiryDate.touched
                    ? validationState.expiryDate.isValid ? 'border-green-500 focus:border-green-500' : 'border-red-500 focus:border-red-500'
                    : 'border-gray-300 focus:border-orange'
                }`}
                maxLength={5} autoComplete='cc-exp' onBlur={handleExpiryBlur}
              />
              {validationState.expiryDate.touched && validationState.expiryDate.isValid && (
                <div className='absolute top-1/2 right-3 -translate-y-1/2'><CheckmarkIcon /></div>
              )}
            </motion.div>
          </div>
          <div>
            <div className='mb-1 flex items-center gap-1'>
              ...
              <div className='relative'>
                <Button type='button' onClick={toggleCvvTooltip} variant='ghost' animated={false} className='flex items-center' aria-label='CVV là gì?'>
                  <InfoIcon />
                </Button>
            ...
            <motion.div className='relative' animate={shakeFields.cvv ? 'shake' : ''} variants={shakeAnimation}>
              <Input
                type='password' placeholder={cardType === 'amex' ? '••••' : '•••'} register={register} name='cvv'
                errorMessage={errors.cvv?.message}
                classNameInput={`w-full rounded-lg border px-2 py-2 md:px-3 md:py-3 pr-8 md:pr-10 focus:outline-hidden ${
                  validationState.cvv.touched
                    ? validationState.cvv.isValid ? 'border-green-500 focus:border-green-500' : 'border-red-500 focus:border-red-500'
                    : 'border-gray-300 focus:border-orange'
                }`}
                maxLength={cardType === 'amex' ? 4 : 3} autoComplete='cc-csc' onFocus={handleCvvFocus} onBlur={handleCvvBlur}
              />
