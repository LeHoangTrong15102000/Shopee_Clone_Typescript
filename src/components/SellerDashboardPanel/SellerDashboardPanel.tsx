    ...
    <div className={classNames('rounded-lg bg-white dark:bg-slate-800 shadow p-4', className)}>
      ...
      <div className='grid grid-cols-2 gap-3 mb-4'>
        ...
        <div className='rounded-lg bg-green-50 dark:bg-green-900/20 p-3 text-center'>
          <p className='text-2xl font-bold text-green-600 dark:text-green-400'>{formatCurrency(metrics.today_revenue)}</p>
        ...
        <div className='mb-3'>
          ...
          <div className='space-y-1 max-h-40 overflow-y-auto'>
            {orderNotifications.slice(0, 10).map((notif, index) => (
              <div key={`${notif.order_id}-${index}`} className='flex items-center gap-2 rounded bg-gray-50 dark:bg-slate-700 px-2 py-1.5 text-xs'>
                <span>📦</span>
        ...
        <div>
          ...
          <div className='space-y-1 max-h-40 overflow-y-auto'>
            {qaNotifications.slice(0, 10).map((notif, index) => (
              <div key={`${notif.product_name}-${index}`} className='flex items-center gap-2 rounded bg-gray-50 dark:bg-slate-700 px-2 py-1.5 text-xs'>
                <span>💬</span>
                <div className='flex-1 min-w-0'>
                  <p className='text-gray-700 dark:text-gray-200 truncate'>{notif.user_name}: {notif.question_preview}</p>
                  <Link
